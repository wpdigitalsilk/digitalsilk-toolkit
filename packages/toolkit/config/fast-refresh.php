...
// phpcs:ignoreFile
/**
 * This file is generated by digitalsilk-toolkit and copied into your dist folder, do not modifies this file directly.
 */
namespace DSToolkit;
...
if ( ! defined( 'SCRIPT_DEBUG' ) || ! SCRIPT_DEBUG ) {
	...
	wp_die(
		sprintf(
			"You're using <a href='%s' target='_blank'>digitalsilk-toolkit</a>'s
			...
			"https://github.com/wpdigitalsilk/digitalsilk-toolkit/tree/develop/packages/toolkit",
			"https://github.com/wpdigitalsilk/digitalsilk-toolkit/tree/develop/packages/toolkit#hmr-and-fast-refresh"
		)
	);
}
...
add_action( 'init',
	...
	function() {
		...
		if ( ! $dist_url || ! $dist_path ) {
			wp_die( 'digitalsilk-toolkit error: you must defined call DSToolkit\set_dist_url_path with the URL and path to dist folfer to get fast refresh to work' );
		}
		...
		wp_register_script(
			'digitalsilk-toolkit-react-fast-refresh-entry',
		...
		wp_register_script(
			'digitalsilk-toolkit-react-refresh-runtime',
			$react_fast_refresh_runtime,
			[],
			filemtime( $react_fast_refresh_runtime_path ),
		);

		// the hmr runtime is unique to a digitalsilk-toolkit build
		$prefix          = $project . '-';
		$hmr_runtime_key = $prefix . 'digitalsilk-toolkit-hmr-runtime';
		...
		if ( $script ) {
			...
			if( !in_array( 'digitalsilk-toolkit-react-refresh-runtime', $script->deps ) ){
				$script->deps[] = 'digitalsilk-toolkit-react-refresh-runtime';
			}

			if( !in_array( 'digitalsilk-toolkit-react-fast-refresh-entry', $script->deps ) ){
				$script->deps[] = 'digitalsilk-toolkit-react-fast-refresh-entry';
			}
...
add_action(
	...
	function() {
		...
		wp_enqueue_script( 'digitalsilk-toolkit-react-refresh-runtime' );
